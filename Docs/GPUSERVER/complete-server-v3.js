...
const THUMBNAIL_CONFIG = {
    ...
    DEFAULT_TIMESTAMP: 5,
    DEFAULT_FORMAT: 'jpg', // デフォルトフォーマット
    SUPPORTED_FORMATS: ['jpg', 'jpeg', 'png', 'webp'], // サポート形式
    WEBP_QUALITY: 85,
    JPEG_QUALITY: 90,
    PNG_COMPRESSION: 6
};
...
// サムネイル設定取得API（新規追加）
app.get('/thumbnail/config', (req, res) => {
    res.json({
        supportedFormats: THUMBNAIL_CONFIG.SUPPORTED_FORMATS,
        defaultFormat: THUMBNAIL_CONFIG.DEFAULT_FORMAT,
        defaultSize: THUMBNAIL_CONFIG.DEFAULT_SIZE,
        defaultTimestamp: THUMBNAIL_CONFIG.DEFAULT_TIMESTAMP,
        quality: {
            webp: THUMBNAIL_CONFIG.WEBP_QUALITY,
            jpeg: THUMBNAIL_CONFIG.JPEG_QUALITY,
            png: THUMBNAIL_CONFIG.PNG_COMPRESSION
        },
        description: {
            formats: {
                jpg: 'JPEG形式 - 高圧縮率、幅広い互換性',
                png: 'PNG形式 - 可逆圧縮、透明度対応',
                webp: 'WebP形式 - 高効率圧縮、最新ブラウザ対応'
            },
            usage: {
                web: 'Webサイトやブログでの表示に最適',
                app: 'アプリケーション内での画像表示',
                archive: 'アーカイブや保存用途'
            }
        }
    ...
    try {
        ...
        res.json({
            ...
            jobData: {
                ...
                thumbnailGeneration: jobData.generateThumbnail,
                thumbnailFormat: jobData.thumbnailFormat
            },
            timestamps: {
    ...
    try {
        ...
        const jobsWithProgress = await Promise.all(activeJobs.map(async (job) => {
            ...
            return {
                ...
                thumbnailGeneration: job.data?.generateThumbnail || false,
                thumbnailFormat: job.data?.thumbnailFormat || THUMBNAIL_CONFIG.DEFAULT_FORMAT,
                createdAt: job.timestamp,
                processedOn: job.processedOn,
                estimatedTimeRemaining: calculateEstimatedTimeRemaining(job, progress),
                currentTime: job.processedOn ? Date.now() - job.processedOn : 0
...
// サムネイル形式変換API（新規追加）
app.post('/thumbnail/convert', async (req, res) => {
    try {
        const { inputPath, outputPath, targetFormat, quality } = req.body;
        ...
        if (!outputPath) {
            return res.status(400).json({ 
                success: false,
                error: '出力パスが指定されていません' 
            });
        }
        
        let validatedFormat;
        try {
            validatedFormat = validateThumbnailFormat(targetFormat);
        } catch (error) {
            return res.status(400).json({
                success: false,
                error: error.message,
        ...
        console.log(`🔄 サムネイル形式変換開始: ${path.basename(inputPath)} → ${validatedFormat}`);
        
        let convertedPath;
        
        if (validatedFormat === 'webp') {
            // WebP変換はsharpを使用
            try {
                const webpQuality = quality || THUMBNAIL_CONFIG.WEBP_QUALITY;
                await sharp(inputPath)
                    .webp({ quality: webpQuality, effort: 6 })
                    .toFile(outputPath);
                convertedPath = outputPath;
            } catch (sharpError) {
                throw new Error(`WebP変換失敗: ${sharpError.message}`);
            }
        } else {
            // その他の形式はFFmpegを使用
            convertedPath = await new Promise((resolve, reject) => {
                const outputOptions = ['-y'];
                
                switch (validatedFormat) {
                    case 'jpg':
                        const jpegQuality = quality || THUMBNAIL_CONFIG.JPEG_QUALITY;
                        outputOptions.push('-q:v', jpegQuality.toString());
                        break;
                    case 'png':
                        const pngCompression = quality || THUMBNAIL_CONFIG.PNG_COMPRESSION;
                        outputOptions.push('-compression_level', pngCompression.toString());
                        break;
                }
                ...
                ffmpeg(inputPath)
                    ...
                    .on('error', (err) => reject(new Error(`形式変換失敗: ${err.message}`)))
                    .run();
            });
        }
        
        if (!convertedPath || !fsSync.existsSync(convertedPath)) {
            throw new Error('変換後ファイルが見つかりません');
        }
        
        const stats = await fs.stat(convertedPath);
        const sizeMB = parseFloat((stats.size / 1024 / 1024).toFixed(3));
        
        console.log(`✅ サムネイル形式変換完了: ${path.basename(convertedPath)} (${sizeMB}MB)`);
        ...
        res.json({
            ...
            sizeMB: sizeMB,
            message: `サムネイルが${validatedFormat}形式に変換されました`
        });
        
    } catch (error) {
        console.error('サムネイル形式変換エラー:', error);
        res.status(500).json({ 
            success: false, 
            error: error.message,
            supportedFormats: THUMBNAIL_CONFIG.SUPPORTED_FORMATS
...
// 一括サムネイル生成API（新規追加）
app.post('/thumbnails/batch-generate', async (req, res) => {
    try {
        const { videoFiles, outputDir, format = THUMBNAIL_CONFIG.DEFAULT_FORMAT, timestamp = THUMBNAIL_CONFIG.DEFAULT_TIMESTAMP } = req.body;
        
        if (!videoFiles || !Array.isArray(videoFiles) || videoFiles.length === 0) {
            return res.status(400).json({ 
                success: false,
                error: '動画ファイルリストが指定されていません' 
            });
        }
        ...
        let validatedFormat;
        try {
            validatedFormat = validateThumbnailFormat(format);
        } catch (error) {
            return res.status(400).json({
                success: false,
                error: error.message,
        ...
        console.log(`📸 一括サムネイル生成開始: ${videoFiles.length}ファイル (${validatedFormat}形式)`);
        ...
        for (const videoFile of videoFiles) {
            try {
                ...
                const thumbnailFilename = `${videoBasename}_thumb.${validatedFormat}`;
                const thumbnailPath = path.join(outputDir, thumbnailFilename);
                
                const metadata = await getVideoMetadata(videoFile);
                const generatedPath = await generateThumbnail(videoFile, thumbnailPath, timestamp, metadata, validatedFormat);
        ...
        console.log(`✅ 一括サムネイル生成完了: 成功${successCount}件, 失敗${failCount}件`);
        ...
        res.json({
            ...
            message: `一括サムネイル生成完了: ${successCount}/${videoFiles.length}件成功`
        });
...
app.listen(SERVER_CONFIG.PORT, async () => {
    ...
    console.log('📸 サムネイル対応形式:', THUMBNAIL_CONFIG.SUPPORTED_FORMATS.join(', '));
    console.log('📊 ダッシュボード: http://localhost:' + SERVER_CONFIG.PORT + '/dashboard.html');
    console.log('🎨 サムネイル機能: WebP・JPG・PNG対応');
    
    await initializeHistoryFile();
});.error('ファイルサイズ取得エラー:', error);
        return 0;
    }
};
...
// フォーマット検証関数
const validateThumbnailFormat = (format) => {
    const normalizedFormat = format.toLowerCase();
    if (!THUMBNAIL_CONFIG.SUPPORTED_FORMATS.includes(normalizedFormat)) {
        throw new Error(`サポートされていないサムネイル形式: ${format}. サポート形式: ${THUMBNAIL_CONFIG.SUPPORTED_FORMATS.join(', ')}`);
    }
    return normalizedFormat === 'jpeg' ? 'jpg' : normalizedFormat; // jpeg を jpg に正規化
};
...
const generateThumbnail = async (inputFile, outputPath, timestamp = THUMBNAIL_CONFIG.DEFAULT_TIMESTAMP, metadata = null, format = THUMBNAIL_CONFIG.DEFAULT_FORMAT, customSize = null) => {
    try {
        // フォーマット検証
        const validatedFormat = validateThumbnailFormat(format);
        
        console.log(`📸 サムネイル生成開始: ${path.basename(inputFile)} → ${path.basename(outputPath)} (${validatedFormat}形式)`);
        ...
        // その他の形式（JPG、PNG等）はFFmpegを使用
        return await generateStandardThumbnail(inputFile, outputPath, timestamp, metadata, validatedFormat, customSize);
        
    } catch (error) {
        console.error(`⚠️ サムネイル生成失敗 (${format}):`, error.message);
        return null;
    }
};

// 標準サムネイル生成（FFmpeg使用）
const generateStandardThumbnail = async (inputFile, outputPath, timestamp, metadata, format, customSize) => {
    ...
    if (!customSize && metadata && metadata.streams) {
        ...
        if (videoStream && videoStream.width && videoStream.height) {
            ...
            if (isPortrait) {
                ...
                videoFilters.push('scale=720:1280:force_original_aspect_ratio=decrease');
                console.log(`📱 縦型動画検出: ${videoStream.width}x${videoStream.height} → サムネイル: ${thumbnailSize}`);
            } else {
    ...
    return new Promise((resolve, reject) => {
        ...
        switch (format) {
            case 'jpg':
                ...
                outputOptions.push('-f', 'image2');
                break;
            case 'png':
                outputOptions.push('-c:v', 'png');
                outputOptions.push('-compression_level', THUMBNAIL_CONFIG.PNG_COMPRESSION.toString());
                break;
            default:
                // デフォルトはJPEG設定
                outputOptions.push('-q:v', THUMBNAIL_CONFIG.JPEG_QUALITY.toString());
        ...
        command.outputOptions(outputOptions)
            ...
            .on('end', () => {
                ...
                console.log(`✅ ${format.toUpperCase()}サムネイル生成完了: ${path.basename(outputPath)} (${thumbnailSize}, ${mode})`);
                resolve(outputPath);
            })
            .on('error', (err) => {
                console.error(`❌ ${format.toUpperCase()}サムネイル生成エラー:`, err.message);
                resolve(null); // エラーでもnullを返してチェーンを継続
            })
            .run();
    });
};

// WebPサムネイル生成（sharp使用・改善版）
                ...
                command.output(tempJpegPath)
                    .on('end', async () => {
                        try {
                            ...
                            console.error(`❌ WebP変換エラー:`, sharpError.message);
                            console.log(`🔄 JPEGフォールバックを試行中...`);
                            
                            // WebP変換失敗時のJPEGフォールバック
                            try {
                                const jpegFallbackPath = outputPath.replace('.webp', '.jpg');
                                await fs.copyFile(tempJpegPath, jpegFallbackPath);
                                await fs.unlink(tempJpegPath).catch(() => {});
                                console.log(`⚠️ WebP変換失敗、JPEGで保存: ${path.basename(jpegFallbackPath)}`);
                                resolve(jpegFallbackPath);
                            } catch (copyError) {
                                await fs.unlink(tempJpegPath).catch(() => {});
                                console.error(`❌ JPEGフォールバックも失敗:`, copyError.message);
                                reject(copyError);
                            }
                        }
                    })
                    .on('error', async (err) => {
                        await fs.unlink(tempJpegPath).catch(() => {});
                        console.error(`❌ FFmpeg WebP前処理エラー:`, err.message);
                        reject(err);
                    })
                    .run();
            } catch (error) {
...
app.get('/health', async (req, res) => {
    try {
        ...
        res.json({ 
            ...
            nfs: fsSync.existsSync(PATH_CONFIG.NFS_MOUNT) ? 'Mounted' : 'Not Mounted',
            orientation: 'Portrait & Landscape Support',
            thumbnailFormats: THUMBNAIL_CONFIG.SUPPORTED_FORMATS,
            version: 'Complete-API v1.1'
        });
    } catch (error) {
        res.status(500).json({ 
...
app.post('/generate-thumbnail', async (req, res) => {
    try {
        const { 
            ...
            size = THUMBNAIL_CONFIG.DEFAULT_SIZE, 
            format = THUMBNAIL_CONFIG.DEFAULT_FORMAT 
        } = req.body;
        ...
        console.log(`📸 サムネイル生成API開始: ${path.basename(inputFile)} → ${path.basename(outputPath)} (${validatedFormat}, ${size})`);
        
        const thumbnailPath = await generateThumbnail(inputFile, outputPath, timestamp, null, validatedFormat, size);
        
        if (!thumbnailPath) {
            return res.status(500).json({ 
                success: false, 
                error: `サムネイル生成に失敗しました (${validatedFormat}形式)`,
        ...
        const actualFormat = path.extname(thumbnailPath).slice(1); // 実際に生成されたファイルの拡張子
        
        console.log(`✅ サムネイル生成API成功: ${path.basename(thumbnailPath)} (${thumbnailSizeMB}MB, ${actualFormat})`);
        ...
        res.json({
            ...
            message: `サムネイルが正常に生成されました (${actualFormat}形式)`
        });
...
app.post('/upload-and-transcode', upload.single('video'), async (req, res) => {
    try {
        ...
        const { 
            ...
            generateThumbnail, 
            thumbnailTimestamp, 
            thumbnailFormat = THUMBNAIL_CONFIG.DEFAULT_FORMAT, // デフォルト値を明示
            metadata 
        } = req.body;
        ...
        // サムネイルフォーマット検証
        let validatedThumbnailFormat = THUMBNAIL_CONFIG.DEFAULT_FORMAT;
        if (shouldGenerateThumbnail && thumbnailFormat) {
            try {
                validatedThumbnailFormat = validateThumbnailFormat(thumbnailFormat);
                console.log(`📸 サムネイルフォーマット検証: ${thumbnailFormat} → ${validatedThumbnailFormat}`);
            } catch (error) {
                console.warn(`⚠️ 無効なサムネイルフォーマット: ${thumbnailFormat}, デフォルト(${THUMBNAIL_CONFIG.DEFAULT_FORMAT})を使用`);
                validatedThumbnailFormat = THUMBNAIL_CONFIG.DEFAULT_FORMAT;
            }
        }
        
        console.log(`📸 サムネイル生成設定: 有効=${shouldGenerateThumbnail}, フォーマット=${validatedThumbnailFormat}`);
        ...
        const job = await transcodeQueue.add('transcode', {
            ...
            thumbnailFormat: validatedThumbnailFormat
        });
        ...
        res.json({ 
            ...
            systemStatus,
            thumbnailGeneration: {
                enabled: shouldGenerateThumbnail,
                format: validatedThumbnailFormat,
                timestamp: parseInt(thumbnailTimestamp) || THUMBNAIL_CONFIG.DEFAULT_TIMESTAMP,
                supportedFormats: THUMBNAIL_CONFIG.SUPPORTED_FORMATS
            },
            progressUrl: `/job/${job.id}/progress`,
